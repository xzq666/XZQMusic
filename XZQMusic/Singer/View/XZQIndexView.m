...
@interface XZQIndexView ()

@property(nonatomic,strong) XZQSectionIndicatorView *indicatorView;
...
//    self.selectedImageView.frame = CGRectZero;
    ...
    // 以 '热' 字为标准作为其他字母的标准宽高
    self.itemMaxSize = [@"热" boundingRectWithSize:CGSizeMake(SCREEN_WIDTH, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [UIFont boldSystemFontOfSize:self.titleFontSize]} context:nil].size;
    // 标题视图布局
...
//        self.selectedImageView.frame = CGRectZero;
    ...
    if (newIndex >= 0 && newIndex < self.itemsViewArray.count) {
        ...
        newItemLabel.textColor = TitleSelectColor;
        // 处理选中圆形
        // 圆直径
        BOOL isLetter = YES;        // 是否是字母
        unichar firstLetter = [newItemLabel.text characterAtIndex:0];
        if ((firstLetter >= 'a' && firstLetter <= 'z')
            || (firstLetter >= 'A' && firstLetter <= 'Z') || [newItemLabel.text isEqualToString:@"热"]) {
        ...
        if (self.isCallback && self.delegate && [self.delegate respondsToSelector:@selector(selectedSectionIndexTitle:atIndex:)]) {
            ...
            if (isLetter) {
                ...
                if (!self.indicatorView) {
                    self.indicatorView = [[XZQSectionIndicatorView alloc] initWithFrame:CGRectMake(0, 0, 50, 50)];
                }
...
//        _selectedImageView.backgroundColor = TitleSelectColor;
//    }
